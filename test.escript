...
punycode_encode_tests() ->
    test_each("test/punycode_*", fun (Test) ->
                {proplists:get_value(punycode, Test),
                 punycode:encode(proplists:get_value(unicode, Test))}
        end).

idna_to_ascii_tests() ->
    test_each("test/idna_*", fun (Test) ->
                {proplists:get_value(output, Test),
                 idna:to_ascii(xmerl_ucs:from_utf8(proplists:get_value(input, Test)))}
        end).
